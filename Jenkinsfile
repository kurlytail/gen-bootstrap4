...
pipeline {
    ...
    stages {
        stage('Prepare env') {
            ...
            steps {
                script {
                    loadLibrary()
                    env['NPM_VERSION_NUMBER'] = getNpmVersion 'kurlytail/gen-bootstrap4/master', params.BUILD_VERSION_PREFIX, params.BUILDS_OFFSET
    ...
    post {
        always {
            slackSend message: "gen-bootstrap4 build ${env.NPM_VERSION_NUMBER} - Status ${currentBuild.result} - ${env.BUILD_URL}"
        }
    }
}
